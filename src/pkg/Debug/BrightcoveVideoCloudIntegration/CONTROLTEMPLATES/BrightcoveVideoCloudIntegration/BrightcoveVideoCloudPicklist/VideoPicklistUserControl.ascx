...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var chooser = '<%= Request.QueryString[BrightcoveVideoCloudIntegration.VideoCloudWebPart.QueryStringKeyAsyncChooserText] %>';
    ...
    function FillAvailablePicklist(data) {
        ...
        for (var i = 0; i < vcAsyncVideoResults.length; i++) {
            buffer.push(CreatePicklistItem(vcAsyncVideoResults[i]));
        }

        $('.videosAvailable').html(buffer.join(''));
        $('#picklistPaging').html(vcAsyncVideoPaging);
        $('#picklistPaging a').each(function () {
            $(this).attr('href', 'javascript:$.ajax({url:' + $(this).attr('href') + ', success: FillAvailablePicklist});');
        });
    }
    ...
    function CreatePicklistItem(video) {
        var result = 
            '<div class="item" videoId="' + video.id + '">' + 
            '    <div class="videoCheckbox"><input type="checkbox" onclick="SelectToggleItem(this);" /></div>' + 
    ...
    function SelectToggleItem(itemCheckbox) {
        var parentElem = $(itemCheckbox).parent().parent();
        var selected = null;
...
<div id="divPicklist" style="display:none;">
    <div class="picklist available">
        ...
        <div class="searchBox">
            <input class="searchBox_default_text" type="text" value="search video" onkeypress="return disableEnterKey(event);" onchange="return DoSearch(this);" onfocus="if(this.value=='search video'){this.value='';$(this).removeClass('searchBox_default_text');}" /><button onclick="return DoSearch();">&nbsp;</button>
        </div>
